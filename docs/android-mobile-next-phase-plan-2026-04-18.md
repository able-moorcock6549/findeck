...
- `v0.4.15` + `v0.4.16`：品牌与首页顶区已完成减噪，`控制台 / Console` 已收敛成更轻的 `控制台 / Console`，主界面顶区最终收成仅保留 3 个统计标签
...
- 会话列表顶部当前确实存在“控制台 / 项目导航 / 项目按更新时间分组”这类多层说明，信息密度偏高，不利于手机扫读。
...
把“控制台”和“项目文件夹”等重说明文案收敛成更适合手机端的短词。
...
- `v0.4.15`：统一品牌文案，默认只保留“控制台 / Console”，去掉“控制台”等过重命名
...
- `v0.4.16`：删除“控制台 / 项目导航 / 项目按更新时间分组”这类说明性文字，只保留 3 个统计标签层表达
