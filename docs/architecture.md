...
- `apps/android`: native Android client with direct API calls and a native Compose session-detail experience
...
- Native Compose for authentication, server management, inbox, session list, settings, and session detail
- Streaming run state is consumed from the backend SSE endpoint, with Android-side polling fallback when the stream is unavailable

This keeps the APK self-contained while preserving a mobile-first AI chat experience.
