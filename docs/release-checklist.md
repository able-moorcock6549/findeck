...
- `npm run test --workspace @findeck/server -- src/__tests__/pairing-store.test.ts src/__tests__/pairing.test.ts src/__tests__/session-archive.test.ts src/__tests__/composer-ux.test.ts src/__tests__/runtime.test.ts src/__tests__/runtime-route.test.ts src/__tests__/run-approvals.test.ts src/__tests__/files-download.test.ts`
...
- keep release responsibilities split clearly:
  - `skills/findeck-release/` is for GitHub repo release only
...
- ensure generated launchd launcher scripts in `~/Library/Application Support/findeck/launchd/` are not copied back into the repo
